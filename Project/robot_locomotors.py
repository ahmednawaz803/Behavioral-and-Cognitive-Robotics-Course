...
class WalkerBase(MJCFBasedRobot):
  ...
  def calc_state(self):
    ...
    for p in self.parts.values():
      if ('pelvis' or 'torso' in p.name):   # partname can be “pevis” or “torso”
        forcear = p.speed()                 # return a 3D vector with the speed of teh part with respect to the 3 axes
      else:
        forcear = [0,0,0]                   # For the robot configurations that does not have pelvis has no effect!
    
    force = [np.random.uniform(-rr, rr) for rr in forcear]      # Generates a number depending upon the velocity of pelvis
    if force != None:
      for p in self.parts.values():
        if ('pelvis' or 'torso' in p.name):           # Here force will only be applied to pelvis, not the whole body
          p.setExternalForce(force,p.pose().xyz())
